...
- **Veículos** now synced from `VEICULOS` sheet to `veiculos_sync` Supabase table; fatura detail shows `Veículo - Matrícula` for VIATURA destinations.
- **Fatura photos** integrated via Google Drive public folder; `syncFaturaPhotoUrls()` fills `Foto_URL` column, dashboard shows PDF thumbnail + link in expanded detail.
- Next step:
  - Run `syncFaturaPhotoUrls()` in GAS editor to populate `Foto_URL` for existing faturas
  - Run SQL migration `014_add_foto_url_to_faturas.sql` and `Sync-SheetsToSupabase.ps1 -Entity faturas -Apply`
  - Run SQL migration `013_create_veiculos_sync.sql` and `Sync-SheetsToSupabase.ps1 -Entity veiculos -Apply`
